...
### 4. PreLLMRequestHandler - LLM请求前处理

**触发时机**：构建 LLM 请求 JSON 后，发送给 LLM API 之前

**适用场景**：
- 修改请求参数（temperature、max_tokens 等）
- 动态添加/修改系统提示词
- 修改用户消息内容
- 拦截请求（直接返回自定义响应）
- 记录/分析请求日志
...
Api.RegisterPreLLMRequestHandler(ctx =>
{
    ...
    try
    {
        ...
        // 示例1：根据时间调整 temperature（夜间降低随机性）
        ...
        // 示例3：根据消息复杂度调整 max_tokens
        if (!string.IsNullOrEmpty(ctx.UserMessage))
        {
            if (ctx.UserMessage.Length < 10)  // 简单问题
            {
                request["max_tokens"] = 512;
                isModified = true;
            }
            else if (ctx.UserMessage.Length > 200)  // 复杂问题
            {
        ...
        {
            // 构造 AI 回复 JSON（注意：不是完整的 OpenAI 响应格式）
            var testReply = new
            {
                messages = new[]
                {
            ...
            {
                IsIntercepted = true,
                InterceptedResponse = JsonConvert.SerializeObject(testReply)
            };
        }
        ...
        if (isModified)
        {
            ...
            {
                IsModified = true,
                ModifiedRequestJson = request.ToString(Formatting.None)
            };
        }
...
**重要说明**：
- 拦截时返回的是 **AI 回复内容 JSON**，不是完整的 OpenAI 响应格式
- 修改请求后，框架会使用修改后的 JSON 发送给 LLM API
- 此拦截器不会影响 `context_persistence.json` 的保存内容

### 5. LLMResponseHandler - AI回复处理
